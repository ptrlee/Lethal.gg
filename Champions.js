...
export function spellDamage() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (i != 0 && level[i-1] == 0) { //handles when ability isn't leveled
            totalDamage[i][0][0] == 0;
            continue;
        }
        ...
        if (i == 0 || !spellsChamp[i][spellString].worksForAPI) {
            ...
            if (condition.TickRate == null) { //burst
                if (condition.DiffType != null) 
                    totalDamage[i][j][0] = sortType(totalDamage[i][j][0],type,champD.armor,champD.mr) + sortType(calculateSpell(champA, champD, condition.DiffType, level,w),condition.DiffType.type,champD.armor,champD.mr)
                else 
            ...
            while (condition.amp != undefined) { //if the amp exists
                ...
                if (ampob.flatAmp != undefined) { //flat bonus damage (eve e)
                    if (ampob.flatAmp.TickRate == undefined) {
                        if (ampob.flatAmp.DiffType != null) {
                            totalDamage[i][j].push(sortType(calculateSpell(champA,champD,ampob.flatAmp,level,w),ampob.flatAmp.type,champD.armor,champD.mr) + sortType(calculateSpell(champA,champD,ampob.flatAmp.DiffType,level,w),ampob.flatAmp.DiffType.type,champD.armor,champD.mr))
                        }
                        else 
                    ...
                    if (ampob.ampMult.statChange != undefined) { //stat change w/ amp 
                        ...
                        */
                        if (ampob.ampMult.statChange.percentileAD != undefined) {
                            champA.baseAD *= 1+ampob.ampMult.statChange.percentileAD[level[ampob.statChange.ADSpell]];
                            champA.bonusAD *= 1+ampob.ampMult.statChange.percentileAD[level[ampob.statChange.ADSpell]];
                        }
...
function calculateSpell(champA, champD, spellsChampSpell, level, w) {
    ...
    if (spellsChampSpell.BADScale != null) {
        if (spellsChampSpell.ADDamage != null) {
            if (spellsChampSpell.ADDamage.length != 18) {
                ...
                if (spellsChampSpell.ADDamageBasedOnOtherAbility != null) 
                    totalDamage += spellsChampSpell.ADDamage[champA.level-1]+spellsChampSpell.BADScale*(champA.bonusAD) + spellsChampSpell.ADDamageBasedOnOtherAbility[level[spellsChampSpell.OtherAbility]-1];
                else
        ...
        if (spellsChampSpell.ADDamage != null) {
            if (spellsChampSpell.ADDamage.length != 18) {
                ...
                if (spellsChampSpell.ADScale.length != 18) {
                    if (spellsChampSpell.ADDamageBasedOnOtherAbility != null) 
                        totalDamage += spellsChampSpell.ADDamage[champA.level-1]+spellsChampSpell.ADScale*(champA.bonusAD+champA.baseAD) + spellsChampSpell.ADDamageBasedOnOtherAbility[level[spellsChampSpell.OtherAbility]-1];
                    else
            ...
            if (spellsChampSpell.APDamage.length != 18) {
                if (spellsChampSpell.APScale.length == undefined) {
                    if (spellsChampSpell.APDamageBasedOnOtherAbility != undefined) {
                        totalDamage += spellsChampSpell.APDamage[level[w]-1]+spellsChampSpell.APScale*(champA.bonusAP) + spellsChampSpell.APDamageBasedOnOtherAbility[level[spellsChampSpell.OtherAbility]];
                    }
                    else
                ...
                if (spellsChampSpell.APScale.length!= 18) {
                    if (spellsChampSpell.APDamageBasedOnOtherAbility != undefined) {
                        totalDamage += spellsChampSpell.APDamage[champA.level-1]+spellsChampSpell.APScale*(champA.bonusAP) + spellsChampSpell.APDamageBasedOnOtherAbility[level[spellsChampSpell.OtherAbility]];
                    }
    ...
    if (spellsChampSpell.multiplyBasedOnOtherAbility != null) 
        totalDamage *= spellsChampSpell.multiplyBasedOnOtherAbility[level[spellsChampSpell.OtherAbility]];
